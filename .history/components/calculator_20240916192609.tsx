...
import { evaluate, pi, exp, log, factorial, sin, cos, tan, asin, acos, atan } from "mathjs";
...
const Calculator: React.FC = () => {
  ...
  const handleCalculate = () => {
    try {
      let expressionToEvaluate = expression.replace(/π/g, "pi").replace(/×/g, "*");
      if (!isRadianMode) {
        ...
        <div className="grid grid-cols-5 gap-2">
          ...
          {["(", ")", "%", "CE", "AC"].map((value, index) => (
            <button
              ...
              onClick={() => value === "AC" ? handleClear() : handleButtonClick(value)}
            >
              {value}
            </button>
          ))}
          ...
          {["sin⁻¹", "cos⁻¹", "tan⁻¹", "Rad", "Deg"].map(
            (func, index) => (
              <button
                key={index}
                className={`p-4 ${func === "Rad" && isRadianMode || func === "Deg" && !isRadianMode ? "bg-yellow-500" : "bg-gray-500"} text-white rounded-lg hover:bg-gray-600 text-xl`}
                onClick={() => handleButtonClick(func)}
              >
                {func}
              </button>
